...
def get_replacements(sentence, word_to_replace):
    ...
    CGBERT = []
    substitution_words = []
    ppdb_model = Ppdb("BERT_resources/ppdb-2.0-tldr")
    fasttext_dico, fasttext_emb = getWordmap("BERT_resources/crawl-300d-2M-subword.vec")
    word_frequency = getWordCount("BERT_resources/SUBTLEX_frequency.xlsx")
    ...
    complex_word_index = words.index(word_to_replace)
    complex_word_tag = words_tag[complex_word_index][1]
    complex_word_tag = preprocess_tag(complex_word_tag)
    cgPPDB = ppdb_model.predict(word_to_replace,complex_word_tag)
    cgBERT = BERT_candidate_generation(word_to_replace, pre_tokens, predicted_top[0].cpu().numpy(), ps, num_selections)     
    CGBERT.append(cgBERT)

    pre_word = substitution_ranking(word_to_replace, mask_context, cgBERT, fasttext_dico, fasttext_emb,word_frequency,cgPPDB,tokenizer,model)
...
if __name__ == "__main__":
    ...
    replacements = get_replacements(sentence, word)
