...
def main():
	...
	with open(target_word_file, "r") as target_f, open(reference_file, "r") as ref_f:
		for target,ref in tqdm(zip( target_f, ref_f)):
			line = line.strip()
...
def parse():
	...
	parser.add_argument('--pretrained-bert',    '-b', help='学習済みの日本語BERTモデル　デフォルト：%(default)s', default='cl-tohoku/bert-base-japanese-whole-word-masking')
