...
def load(args): 
	...
	word2vec = KeyedVectors.load_word2vec_format(args.embedding, binary=False)
	...
	return word2vec, w2v_vocab, mecab_wakati, mecab, language_model, word2level, word2synonym, word2freq, freq_total, simple_synonym, w2v_vocab, bert
...
def pick_candidates(target, most_similar, word2vec, w2v_vocab, word2synonym, candidate_type, cos_threshold, bert, sentence, device):
	candidates = list()
	bert_score = list()
...
# ランキング
def ranking(target, candidates, sentence, word2vec, w2v_vocab, word2freq, freq_total, language_model, attached_words, mecab, word2synonym, ranking_type, scores):
